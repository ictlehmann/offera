 ...
 * The displayed role is resolved from the current session via Auth::user().
 */
...
$userRole = 'Mitglied';
$_sidebarUser = Auth::user();
if ($_sidebarUser && !empty($_sidebarUser['role'])) {
    $_sidebarRole = $_sidebarUser['role'];
    $userRole = function_exists('translateRole') ? translateRole($_sidebarRole) : ucfirst($_sidebarRole);
}
