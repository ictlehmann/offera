            ...
            <div class="modal-content">
                ...
                <form id="support-form" method="POST" action="<?php echo asset('api/submit_support.php'); ?>">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="support-type" class="form-label fw-semibold">Art der Anfrage</label>
                            <select class="form-select" id="support-type" name="request_type" required>
                        ...
                        <div class="mb-3">
                            <label for="support-description" class="form-label fw-semibold">Beschreibung</label>
                            <textarea class="form-control" id="support-description" name="description" rows="4" required placeholder="Beschreibe dein Anliegen..."></textarea>
                        </div>
                        <div id="support-feedback" class="d-none"></div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Abbrechen</button>
                        <button type="submit" id="support-submit-btn" class="btn btn-primary">Senden</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (supportType && supportSubmitBtn) {
            supportType.addEventListener('change', function() {
                if (this.value === 'bug') {
                    supportSubmitBtn.textContent = 'Fehler melden';
                } else if (this.value === '') {
                    supportSubmitBtn.textContent = 'Senden';
                } else {
                    supportSubmitBtn.textContent = 'Beantragen';
                }
            });
        }
        ...
        if (supportForm) {
            supportForm.addEventListener('submit', function(e) {
                ...
                const originalText = supportSubmitBtn.textContent;
                ...
                .then(function(data) {
                    supportFeedback.classList.remove('d-none', 'alert-success', 'alert-danger');
                    if (data.success) {
                        supportFeedback.className = 'alert alert-success';
                        supportFeedback.textContent = data.message || 'Anfrage erfolgreich gesendet!';
                        supportForm.reset();
                        supportSubmitBtn.textContent = 'Senden';
                    } else {
                        supportFeedback.className = 'alert alert-danger';
                        supportFeedback.textContent = data.message || 'Fehler beim Senden der Anfrage.';
                        supportSubmitBtn.textContent = originalText;
                    }
                    supportSubmitBtn.disabled = false;
                })
                .catch(function() {
                    supportFeedback.className = 'alert alert-danger';
                    supportFeedback.textContent = 'Fehler beim Senden der Anfrage.';
                    supportFeedback.classList.remove('d-none');
                    supportSubmitBtn.textContent = originalText;
                    supportSubmitBtn.disabled = false;
                });
            });
            ...
            document.getElementById('support-modal').addEventListener('hidden.bs.modal', function() {
                supportFeedback.className = 'd-none';
